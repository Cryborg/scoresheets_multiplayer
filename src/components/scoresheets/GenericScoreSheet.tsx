...
import { Plus, Trophy, TrendingDown, Dice6 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';

export default function GenericScoreSheet({ sessionId }: { sessionId: string }) {
  ...
  const handleSubmitRound = async (session: GameSessionWithRounds) => {
    ...
    try {
      const response = await fetch(`/api/games/jeu-libre/sessions/${sessionId}/rounds`, {
        method: 'POST',
    ...
    >
      {({ session, gameState }) => (
        <div className="space-y-6">
          <Card className="border-indigo-200 dark:border-indigo-800 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30">
            <CardHeader className="pb-4">
              <CardTitle className="flex items-center gap-3 text-indigo-800 dark:text-indigo-200">
                <div className="p-2 rounded-lg bg-indigo-100 dark:bg-indigo-900/50">
                  <Trophy className="h-5 w-5" />
                </div>
                Scores actuels - {session.session_name}
              </CardTitle>
            ...
            <CardContent>
              <div className="grid gap-3">
                {session.players
                  ...
                  .sort((a, b) => {
                    // Tri selon la direction de score configurée
                    return session.score_direction === 'lower' 
                      ? a.totalScore - b.totalScore  // Score faible gagne (tri croissant)
                      : b.totalScore - a.totalScore; // Score élevé gagne (tri décroissant)
                  })
                  .map((player, index) => (
                    <div key={player.id} className="group">
                      <div className="flex items-center gap-4 p-4 rounded-xl bg-white/70 dark:bg-gray-800/70 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-200">
                        ...
                        <div className="flex items-center gap-2">
                          {player.totalScore > 0 && <TrendingDown className="h-4 w-4 text-indigo-500" />}
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {(() => {
                        ...
                        // Calculer le meilleur score selon la direction configurée
                        const bestScore = session.score_direction === 'lower' 
                          ? Math.min(...playersWithTotals.map(p => p.totalScore))
                          : Math.max(...playersWithTotals.map(p => p.totalScore));
                        ...
                        return (
                          <tr key={player.id} className="border-b border-purple-100 dark:border-purple-800 hover:bg-white/50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            {session.rounds.map((round, roundIndex) => {
                              ...
                              // Déterminer le meilleur score de la manche selon la direction configurée
                              const isBestInRound = round.scores && roundScore !== undefined && (
                                session.score_direction === 'lower' 
                                  ? roundScore === Math.min(...Object.values(round.scores))
                                  : roundScore === Math.max(...Object.values(round.scores))
                              );
